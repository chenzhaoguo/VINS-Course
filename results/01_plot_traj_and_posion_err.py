...
if __name__=="__main__":
    ...
    print "APE.rmse %f m"%numpy.sqrt(numpy.dot(trans_error,trans_error) / len(trans_error))
    print "APE.mean %f m"%numpy.mean(trans_error)
    print "APE.median %f m"%numpy.median(trans_error)
    print "APE.std %f m"%numpy.std(trans_error)
    print "APE.min %f m"%numpy.min(trans_error)
    print "APE.max %f m"%numpy.max(trans_error)

    ###    plot trajectory    ###
    fig = plt.figure(1)
    ax = fig.gca(projection='3d')
    plot_traj(ax, first_stamps, first_xyz_full.transpose().A, '--', 1.0, "red", "groundtruth")
    plot_traj(ax, second_stamps, second_xyz_full_aligned.transpose().A, '-', 1.0, "blue", "estimate_result")
    ...
    fig = plt.figure(2)
    plt.subplot(311)
    plt.plot(time1, pose_x_gt, '--', linewidth=1.0, color="red", label='groundtruth')
    plt.plot(time1, pose_x_est, '-', linewidth=1.0, color="blue", label='estimate_result')
    plt.ylabel('x [m]')
    plt.legend(loc='upper right', fontsize=8, edgecolor='black')
    plt.grid(linestyle="--")
    plt.subplot(312)
    plt.plot(time1, pose_y_gt, '--', linewidth=1.0, color="red")
    plt.plot(time1, pose_y_est, '-', linewidth=1.0, color="blue")
    plt.ylabel('y [m]')
    plt.grid(linestyle="--")
    plt.subplot(313)
    plt.plot(time1, pose_z_gt, '--', linewidth=1.0, color="red")
    plt.plot(time1, pose_z_est, '-', linewidth=1.0, color="blue")
    plt.ylabel('z [m]')
    plt.xlabel('t [s]')
    plt.grid(linestyle="--")
