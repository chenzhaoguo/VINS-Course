...
file_traj_gt = os.path.abspath('.') + '/groundtruth_tum.txt'
...
## roll
ax11.plot(euler_gt_time_relative, euler_gt[:,0], linestyle='--', linewidth=1.0, color='r', label='groundtruth')
ax11.plot(euler_est_time_relative, euler_estimate[:,0], linestyle='-', linewidth=0.8, color='b', label='estimate_result')
...
ax12.grid(linestyle="--")
## yaw
ax13.plot(euler_gt_time_relative, euler_gt[:,2], linestyle='--', linewidth=1.0, color='r')
ax13.plot(euler_est_time_relative, euler_estimate[:,2], linestyle='-', linewidth=0.8, color='b')
...
ax2.set_ylabel('orientation error [deg]', fontsize=10)
ax2.tick_params(labelsize=9)
ax2.legend(loc='upper right', fontsize=8, edgecolor='w')
ax2.grid(linestyle="--")
## add small figure
insert_ax = fig2.add_axes([0.58, 0.28, 0.3, 0.25])
insert_ax.plot(time, diff_euler[:, 0], linewidth=1.0, color='r', label='roll')
...
insert_ax.set_xlim(0, 20)
insert_ax.set_ylim(-5.0, 1.0)
